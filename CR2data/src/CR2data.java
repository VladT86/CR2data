...
public class CR2data {
	...
	public static void main(String args[]) {
		...
		getAllFiles(path,fileList,ext);
		try {
			for (File f:fileList){
				{
					CR2parse cr2file = new CR2parse(f.getAbsoluteFile());
					photo p = new photo(f.toString(),cr2file.getLensType(), cr2file.getISO(), cr2file.getFocalLength(),cr2file.getExpTime(),cr2file.getExpType(),cr2file.getMetering());
					photos.add(p);
				}	
			}
		}
	...
	private static void getAllFiles(String directoryName, ArrayList<File> files, String ext){
		File directoy = new File(directoryName);
		File[] fList = directoy.listFiles();
		
		for (File f:fList){
			if (f.isFile()){
				if (f.getName().substring(f.getName().length()-ext.length()).toString().compareTo(ext)==0) 
					{
					files.add(f);
					}
			}
			else if (f.isDirectory()) {
				getAllFiles(f.getAbsolutePath(),files,ext);
			}
		}
	}
